...
function rowIndexInLane(node: WorkflowCanvasNode): number {
  const rows = laneRowNodes(node.span_lanes[0], props.nodes);
  const idx = rows.findIndex((r) => r.render_key === node.render_key);
  return idx >= 0 ? idx : node.sort_in_lane;
}
...
function rowTopInLane(
  ...
  const rowCount = laneRowNodes(laneIndex, props.nodes).length;
  if (rowCount === 0) return null;
  const groupHeight = rowCount * nodeHeight + (rowCount - 1) * ROW_GAP;
  const startY = (laneHeight - groupHeight) / 2;
  return startY + rowIndex * (nodeHeight + ROW_GAP);
}
...
function rowTopOnTrack(
  ...
  const laneEl = laneRefs.value[laneIndex];
  if (!laneEl) return null;
  const rel = rowTopInLane(laneIndex, rowIndex, nodeHeight, laneEl.offsetHeight);
...
async function updateLayout() {
  ...
  for (const laneIdx of allLaneIndexes.value) {
    maxRowCount = Math.max(maxRowCount, laneRowNodes(laneIdx, props.nodes).length);
  }
  const minLaneHeight = maxRowCount * nodeHeight + (maxRowCount - 1) * ROW_GAP;
  for (const laneIdx of allLaneIndexes.value) {
  ...
  for (const node of layoutColumnNodes.value) {
    ...
    if (!laneEl) continue;
    const topY = rowTopInLane(node.span_lanes[0], rowIndexInLane(node), measuredHeight, laneEl.offsetHeight);
  ...
  for (const node of layoutSpanNodes.value) {
    ...
    const topY = rowTopOnTrack(node.span_lanes[0], rowIndexInLane(node), measuredHeight, trackRect);
...
function drawConnectors(trackEl: HTMLElement) {
  ...
  for (const { from, to } of collectBlockingConnectorPairs(
    ...
    const x1 = lr.right - trackRect.left;
    const y1 = lr.top + lr.height / 2 - trackRect.top;
    const x2 = rr.left - trackRect.left;
    const y2 = rr.top + rr.height / 2 - trackRect.top;
    if (x2 <= x1 + 4) continue;
    paths.push(bezier(x1, y1, x2, y2));
  }

<template>
  <div class="workflow-canvas workflow-canvas--view">
      ...
      <div ref="trackRef" class="workflow-track" :style="{ minWidth: trackMinWidth }">
        ...
        <div class="workflow-lanes">
          ...
          >
            <div
              ...
              class="workflow-node workflow-node--column workflow-node--view"
              :class="nodeClass(node)"
              :style="columnStyles[node.render_key]"
              @click="emit('node-select', node.node_key)"
            >
              <div class="workflow-node-inner">
                ...
                <div class="workflow-node-text">
                  ...
                  <n-text depth="3" class="workflow-node-meta">{{ nodeMeta(node.node_key) }}</n-text>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="workflow-overlay">
          <div
            ...
            :key="node.render_key"
            :ref="(el) => setNodeRef(node.render_key, el as HTMLElement | null)"
            class="workflow-node workflow-node--span workflow-node--view"
            :class="nodeClass(node)"
...
import {
  buildVersionPrerequisites,
  collectPrerequisiteConnectorPairs,
  collectVersionConnectorPairs,
  formatVersionNodeSchedule,
} from '@/utils/versionWorkflowLayout';
...
const progressByKey = computed(() => new Map(props.progress.map((p) => [p.node_key, p])));
...
function nodeMeta(nodeKey: string): string {
  ...
  const schedule = prog ? formatVersionNodeSchedule(prog) : '';
...
async function updateLayout() {
  ...
  const spStyles: Record<string, { left: string; top: string }> = {};
  for (const node of layoutColumnNodes.value) {
  ...
  for (const node of layoutSpanNodes.value) {
    ...
    spStyles[node.render_key] = {
      left: `${leftX}px`,
      top: `${topY}px`,
    };
  }
  columnStyles.value = colStyles;
  spanStyles.value = spStyles;
...
function drawConnectors(trackEl: HTMLElement) {
  ...
  const pairs =
    props.defs.length > 0
      ? collectPrerequisiteConnectorPairs(
          props.nodes ?? [],
          buildVersionPrerequisites(props.defs)
        )
      : collectVersionConnectorPairs(
          props.nodes ?? [],
...
watch(
  () =>
    [
      props.nodes
        .map((n) => `${n.render_key}:${n.sort_in_lane}:${n.span_lanes.join('-')}:${n.state}`)
        .join('|'),
      props.progress.map((p) => `${p.node_key}:${p.assignee_id}:${p.scheduled_start}:${p.scheduled_end}`).join('|'),
    ].join(';'),
  () => void updateLayout()
);
watch([allLaneIndexes, layoutSpanNodes], () => void updateLayout());
</script>

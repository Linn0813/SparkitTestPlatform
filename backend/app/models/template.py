...
class RequirementStatusRule(Base):
    """当前工作流阶段（按节点集合）对应的需求状态推导规则。"""

    __tablename__ = "requirement_status_rules"
...
class ProjectIntegration(Base):
    ...
    version_wecom_webhook_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    version_wecom_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
